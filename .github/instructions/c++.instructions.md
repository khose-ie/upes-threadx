...
- Naming conventions: variables and function parameters use lower_snake_case; function names use lower_snake_case; type names (classes/structs/unions/enums) use PascalCase and camelCase and must end with `_t`, if includes module name, it should be cameCase (e.g. `TheType_t` or `moduleType_t`); member fields use lower_snake_case. Avoid using overly short names even in local scope or loops.
